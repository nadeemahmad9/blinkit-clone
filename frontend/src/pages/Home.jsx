...
import ProductSkeleton from "../components/common/ProductSkeleton"; // Adjust path if needed
...
const Home = () => {
    // Removed unused 'products' state to clean up
    const [loading, setLoading] = useState(true);
    const [recommendedProducts, setRecommendedProducts] = useState([]);
    const [snackProducts, setSnackProducts] = useState([]);
    const [BeveragesProducts, setBeveragesProducts] = useState([]);
        ...
        const loadData = async () => {
            try {
                ...
                const [recommendedData, snacksData, BeveragesData] = await Promise.all([
                    fetchProducts(),           // 1. Recommended
                    fetchProducts("Snacks"),   // 2. Snacks
                    fetchProducts("Beverages") // 3. Beverages
                ]);
    ...
    // This creates an array of 6 items and maps them to Skeletons
