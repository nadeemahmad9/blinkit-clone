...
import { useSearchParams, useNavigate } from "react-router-dom";
import { ArrowLeft, Search as SearchIcon } from "lucide-react";
// ✅ FIX 1: Import your centralized API helper
...
const SearchPage = () => {
    ...
    useEffect(() => {
        const fetchSearchResults = async () => {
            ...
            try {
                if (query) {
                    // ✅ FIX 2: Use api.get (handles base URL automatically)
                    const { data } = await api.get(`/products?search=${query}`);

                    // ✅ FIX 3: Ensure data is an array before setting state
    ...
    return (
        <div className="bg-brand-gray min-h-screen font-sans bg-[#F4F6FB]">
            ...
            <div className="sticky top-0 z-20 bg-white border-b border-gray-200 shadow-sm">
                <div className="max-w-[1280px] mx-auto px-4 py-3 flex items-center gap-3">
                    ...
                    <div className="flex-1">
                        <h1 className="text-lg font-extrabold text-brand-dark truncate text-gray-800">
                            {query ? `Results for "${query}"` : "Search"}
                        </h1>
                        {!loading && products.length > 0 && (
                            <p className="text-xs text-gray-500">{products.length} items found</p>
            ...
            <div className="max-w-[1280px] mx-auto p-4">
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center pt-20 text-gray-400 min-h-[50vh]">
                        ...
                        <button
                            onClick={() => navigate("/")}
                            className="mt-6 text-[#0c831f] font-bold text-sm hover:underline"
                        >
                            Back to Home
                        </button>
                    </div>
                    ...
                    /* REAL DATA GRID */
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                        {/* ✅ FIX 4: Use Optional Chaining (?.) for safety */}
                        {products?.map((prod) => (
                            <ProductCard key={prod._id} product={prod} />
                        ))}
                    </div>
