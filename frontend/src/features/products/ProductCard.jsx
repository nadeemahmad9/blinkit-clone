...
const ProductCard = ({ product }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col gap-2 shrink-0 bg-white hover:shadow-lg transition-shadow duration-300 rounded-lg p-2 border border-gray-200 hover:border-gray-200 relative">
            ...
            {/* ============================================================== */}
            {/* 1. MASTER LINK: Transparent Overlay covering the whole card    */}
            ...
            <div className="flex flex-col gap-1 grow">
                ...
                <div className="flex items-center justify-between mt-auto pt-2">
                    ...
                    {/* ========================================================== */}
                    {/* 2. BUTTONS: z-20 puts them ABOVE the Link (z-10)           */}
                    {/* e.preventDefault() stops the link from firing           */}
                    {/* ========================================================== */}
                    <div className="w-[65px] sm:w-[70px] h-[28px] sm:h-[30px] relative z-20">
                        <AnimatePresence mode="wait">
                            {count === 0 ? (
                                <motion.div
                                    key="add-btn"
                                ...
                                >
                                    <button
                                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); removeFromCart(product._id); }}
                                        className="flex items-center justify-center w-4 h-full cursor-pointer"
                                    >
                                        <Minus size={10} strokeWidth={4} />
                                    </button>
                                    <span className="text-[10px] sm:text-[11px] font-bold">{count}</span>
