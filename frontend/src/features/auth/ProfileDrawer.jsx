...
import {
    X, Package, MapPin, Heart, LogOut,
    ChevronRight, Phone, Mail, User, ShieldCheck, Edit2
} from "lucide-react";
import { useAuthStore } from "../../store/useAuthStore";
import { useNavigate } from "react-router-dom";
                ...
                <>
                    ...
                    >
                        ...
                        <div className="bg-white p-6 pt-12 pb-6 border-b border-gray-100 relative">
                            ...
                            <div className="flex flex-col items-center text-center mt-2">
                                {/* Avatar */}
                                <div className="w-20 h-20 bg-yellow-100 text-yellow-700 rounded-full flex items-center justify-center mb-3 text-3xl font-bold border-4 border-white shadow-sm">
                                    {user.name ? user.name.charAt(0).toUpperCase() : <User size={32} />}
                                </div>

                                {/* Name */}
                                <h2 className="text-xl font-extrabold text-gray-900 capitalize leading-tight">{user.name}</h2>

                                {/* Phone */}
                                <div className="flex items-center gap-1.5 mt-1 mb-3">
                                    <Phone size={12} className="text-gray-400" />
                                    <span className="text-xs text-gray-500 font-bold">{user.phone}</span>
                                </div>

                                {/* ✅ VIEW PROFILE BUTTON (Moved Inside Here) */}
                                <button
                                    onClick={handleGoToProfile}
                                    className="text-xs bg-green-50 text-[#0c831f] font-bold px-4 py-2 rounded-full flex items-center gap-1 hover:bg-green-100 transition-colors"
                                >
                                    View & Edit Profile <Edit2 size={10} />
                                </button>
                            </div>
                        </div>
